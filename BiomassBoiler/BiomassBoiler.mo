package BiomassBoiler
  ...
  package Test
    ...
    model Unnamed5
      ...
      Modelica.Thermal.HeatTransfer.Sources.FixedTemperature fixedTemperature(T=433.15)
        annotation (Placement(transformation(
            ...
            rotation=180,
            origin={110,32})));
      Components.Sources.AirSource airSource1(V_flow_const=0)
        annotation (Placement(transformation(extent={{58,48},{38,68}})));
      Components.Sources.AirSource airSource2(V_flow_const=0.13)
        annotation (Placement(transformation(extent={{70,-72},{50,-52}})));
      Modelica.Blocks.Sources.Constant const(k=2)
        annotation (Placement(transformation(origin={-108,-28},
    extent={{-10,-10},{10,10}})));
      Components.Sources.FuelSource               fuelSource(
        variable_m_flow=true,
        variable_T=false,
        ...
        components_const={0,0.6797,0.1423,0.029})
                                   annotation (Placement(transformation(origin={-46,-46},
              extent={{-10,-10},{10,10}})));
    equation
      connect(gasCombustion.port_a, convection.solid)
        annotation (Line(points={{10,48},{10,24},{-4,24},{-4,18}}, color={191,0,0}));
      connect(gasCombustion.Gc, convection.Gc) annotation (Line(points={{4,47.4},{4,22},{
              12,22},{12,8},{6,8}}, color={0,0,127}));
      connect(convection.fluid, bedCombustion.port_up)
        annotation (Line(points={{-4,-2},{-4,-28},{10,-28},{10,-36}}, color={191,0,0}));
      connect(gasCombustion.port_a, bodyRadiation.port_a)
        annotation (Line(points={{10,48},{10,24},{-44,24},{-44,18}}, color={191,0,0}));
      connect(bodyRadiation.port_b, bedCombustion.port_up) annotation (Line(points={{-44,
              -2},{-44,-30},{10,-30},{10,-36}}, color={191,0,0}));
      connect(gasCombustion.flueGas_inlet_down, bedCombustion.flueGas_outlet) annotation (
         Line(
          points={{15,48},{15,6},{16,6},{16,-36}},
          color={118,106,98},
          thickness=0.5));
      connect(gasCombustion.port_a, bodyRadiation1.port_a)
        annotation (Line(points={{10,48},{10,32},{52,32}}, color={191,0,0}));
      connect(bodyRadiation1.port_b, fixedTemperature.port)
        annotation (Line(points={{72,32},{100,32}}, color={191,0,0}));
      connect(gasCombustion.flueGas_inlet, airSource1.flueGas_outlet) annotation (Line(
          points={{20.2,58},{38,58}},
          color={118,106,98},
          thickness=0.5));
      connect(airSource2.flueGas_outlet, bedCombustion.flueGas_inlet) annotation (Line(
          points={{50,-62},{16,-62},{16,-56}},
          color={118,106,98},
          thickness=0.5));
      connect(const.y, fuelSource.m_flow) annotation (Line(points={{-97,-28},{-62,-28},{
              -62,-40},{-56,-40}}, color={0,0,127}));
      connect(fuelSource.fuel_outlet[1], bedCombustion.fuel_in)
        annotation (Line(points={{-36,-46},{-1,-46}}, color={0,0,0}));
      annotation (
        Icon(coordinateSystem(preserveAspectRatio=false)),
        Diagram(coordinateSystem(preserveAspectRatio=false)),
        experiment(
    ...
    model Unnamed12
      parameter Real m_in = 1;
      Real m_out;
      Real m;
      parameter Real r = 1;
    equation
      m_out = min(m*10, m_in + r);
    //   r = 100*m;
      //m_out =
      der(m) = m_in - m_out;
    end Unnamed12;
    ...
    model Unnamed16
      parameter Integer n = 15;
      parameter Integer n_units = 1;
      parameter Real a[n] = linspace(52.98,1.54,n);
    //   parameter Real b[10] = linspace(67.6,13.6,10);
    //   parameter Real c[20] = linspace(13.6,2.3,20);
      Modelica.Thermal.HeatTransfer.Components.BodyRadiation bodyRadiation[n](each Gr=2.839)
        annotation (Placement(transformation(extent={{-10,-10},{10,10}},
            rotation=90,
            origin={28,64})));
      Components.GasCombustion1 gasCombustion[n](
        ...
        H=1.9)
        annotation (Placement(transformation(extent={{-14,20},{6,40}})));
      Components.CombineHeatTransfer combineHeatTransfer[n](each Gr=1.362, each Gr_n=1.0688)
        annotation (Placement(transformation(extent={{-38,-26},{-18,-6}})));
      SubSystem.BedUnits bedUnits[n](
        n_units=n_units,
        L=0.5,
      ...
      Components.Sources.FuelSource fuelSource(
        ...
        components_const={0.149,0.6797,0.1423,0.029})
        annotation (Placement(transformation(extent={{-66,-66},{-46,-46}})));
      Modelica.Thermal.HeatTransfer.Components.BodyRadiation rad_between_gas[n-1](Gr=4)
        annotation (Placement(transformation(extent={{-10,-10},{10,10}},
            rotation=180,
            origin={16,-16})));
      Components.Sources.AirSource gas_airSource(V_flow_const=0)
      ...
      Modelica.Blocks.Sources.Constant const(k=2.2075)
        annotation (Placement(transformation(extent={{-156,-26},{-136,-6}})));
      Modelica.Thermal.HeatTransfer.Sources.PrescribedTemperature prescribedTemperature(T(start = 1500))
        annotation (Placement(transformation(extent={{-16,80},{4,100}})));
      Components.n_heatTrans n_heatTrans[n-1](
        Gr=0.12,
        G=0.2,                                                 n=n_units)
    ...
    //         origin={-94,90})));
      Modelica.Thermal.HeatTransfer.Celsius.FromKelvin fromKelvin annotation (Placement(
            ...
            rotation=180,
            origin={-82,22})));
      Modelica.Blocks.Discrete.TriggeredSampler triggeredSampler(y_start=1273.15)
        annotation (Placement(transformation(extent={{-54,80},{-34,100}})));
      Modelica.Blocks.Sources.Constant const1(k=380)
    ...
    algorithm
      for i in 1:n loop
        if triggeredSampler.trigger == true then
          break;
        end if;
        if gasCombustion[i].T >= 2573.15 then
    ...
    equation
      connect(temperatureSensor.port, gasCombustion[1].port_a) annotation (Line(points={{-36,22},
              {-20,22},{-20,12},{-4,12},{-4,20}}, color={191,0,0}));
      connect(triggeredSampler.y, prescribedTemperature.T)
        annotation (Line(points={{-33,90},{-18,90}}, color={0,0,127}));
      connect(const1.y, triggeredSampler.u)
        annotation (Line(points={{-81,90},{-56,90}}, color={0,0,127}));
    end Unnamed16;
  ...
  package Components
    ...
    //   for i in 1:size(solution.C, 1) loop
    //
    //     solution.s_in[i] = molar_g_in[i - 4]/V;
    //     solution.s_out[i] = molar_g_out[i - 4]/V;
    //     //     solution.s_out[i] = solution.C[i]/(bedHeight/v_gas);
    //
    //     if noEvent(solution.C[i] <= Modelica.Constants.eps) then
    //       solution.s_out[i] = 0;
    //       aos[i] = 0;
    //       solution.C[i] = 1e-14;
    //     else
    //       solution.gas_out[i] = solution.C[i] / tau;
    //       solution.C[i] = aos[i]/V;
    //     end if;
    //   end for;
    ...
    epsilon = epsilon_0 + min(1, (mf[4]-0.029)/0.00507)*0.03275
    ...
    //   molar_g_out = aos / (bedHeight/v_gas);
      ...
      for i in 5:size(solution.C,1) loop
        ...
        solution.s_in[i] = molar_g_in[i-4] / V;
        solution.s_out[i] = molar_g_out[i-4] / V;
        solution.s_out[i] = solution.mixture.C[i]/(bedHeight/v_gas);
      end for;
